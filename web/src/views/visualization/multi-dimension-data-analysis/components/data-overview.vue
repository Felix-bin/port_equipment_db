...
<script lang="ts" setup>
  ...
  const renderData = computed(() => [
    {
      ...
      value: overviewStats.value.contentProduction,
      prefix: {
        icon: 'icon-edit',
    ...
    },
    {
      ...
      value: overviewStats.value.contentClick,
      prefix: {
        icon: 'icon-thumb-up',
    ...
    },
    {
      ...
      value: overviewStats.value.contentExposure,
      prefix: {
        icon: 'icon-heart',
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await queryDataOverview();
      const data = (response && typeof response === 'object' && 'xAxis' in response) ? response : (response?.data || null);
      
      if (data && data.xAxis && data.data) {
        xAxis.value = data.xAxis;
        
        data.data.forEach((el: any) => {
          if (el.name === '装备入库量') {
            contentProductionData.value = el.value || [];
            overviewStats.value.contentProduction = el.count || 0;
          } else if (el.name === '装备租赁量') {
            contentClickData.value = el.value || [];
            overviewStats.value.contentClick = el.count || 0;
          } else if (el.name === '装备利用率') {
            contentExposureData.value = el.value || [];
            overviewStats.value.contentExposure = el.count || 0;
          } else if (el.name === '活跃租户数') {
            activeUsersData.value = el.value || [];
            overviewStats.value.activeUsers = el.count || 0;
          }
        });
      }
    } catch (err) {
