...
<script lang="ts" setup>
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 响应拦截器可能已经解构了数据
      const data = (response && typeof response === 'object' && 'category_ratio' in response) ? response : (response?.data || null);
      
      if (data && data.category_ratio && data.category_ratio.length > 0) {
        const categoryRatio = data.category_ratio;
        // 使用装备类型作为 x 轴
        xAxis.value = categoryRatio.map((item: any) => item.name);
        
        // 将所有类型的数据放在一个系列中
        textChartsData.value = categoryRatio.map((item: any) => item.value || 0);
        imgChartsData.value = [];
