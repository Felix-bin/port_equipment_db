...
export interface MultiDimensionAnalysisRes {
  data_overview: {
    xAxis: string[];
    data: Array<{ name: string; value: number[]; count: number }>;
  };
  data_chain_growth: Record<string, any>;
  user_actions: {
    data: number[];
  };
...
export function queryMultiDimensionAnalysis() {
  return axios.get<MultiDimensionAnalysisRes>('/api/multi-dimension/analysis');
}
